...
watch(() => props.initialData, (val) => {
    Object.assign(localData, val || {id: null, name: '', value: '', tariffs: []});
}, {immediate: true});
...
const removeTariff = async (tariffId) => {
    ...
    await router.post(route('groups.detach-tariff'), {
        ...
        onSuccess: () => {
            ...
            localData.tariffs = localData.tariffs.filter(t => t.id !== tariffId);
        },
...
<template>
    <form @submit.prevent="submit" class="space-y-4">
        <div>
            ...
            <input v-model="localData.name" type="text" class="border rounded w-full px-3 py-2"/>
            <div v-if="errors && errors.name" class="text-red-600 text-xs">{{ errors.name }}</div>
        </div>
        <div>
            <label class="block text-sm font-medium">Kód / hodnota (unikátní)</label>
            <input v-model="localData.value" type="text" class="border rounded w-full px-3 py-2"/>
            <div v-if="errors && errors.value" class="text-red-600 text-xs">{{ errors.value }}</div>
        </div>
