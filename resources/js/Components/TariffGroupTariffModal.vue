...
const saveUniqueTariffs = () => {
    ...
    uploading.value = true;
    axios.post(route('tariffs.bulk-store'), {
...
<template>
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-8 w-full max-w-xl relative shadow-lg">
            <button class="absolute top-2 right-2 text-gray-400 text-2xl" @click="$emit('close')">&times;</button>
            <h3 class="text-xl font-bold mb-4">Přiřadit službu do skupiny: {{ group.name }}</h3>

            <div class="mb-6 flex gap-4">
                <primary-button @click="tab='vyber'"
                        :class="tab=='vyber' ? 'font-bold text-blue-700 underline' : 'bg-blue-400 text-white px-6 py-2 rounded hover:bg-blue-600'">
                    Vyber službu ze seznamu
                </primary-button>
                <primary-button @click="tab='csv'"
                        :class="tab=='csv' ? 'font-bold text-blue-700 underline' : 'text-gray-500 bg-green-900'">
                    Import služeb z CSV
                </primary-button>
            </div>
            ...
            <div v-if="tab=='vyber'">
                <form @submit.prevent="submit">
                    <select v-model="tariffForm.tariff_id" class="border p-2 w-full rounded mb-4">
                    ...
                    <select v-model="tariffForm.action" class="border p-2 w-full rounded mb-4">
                        <option v-for="a in tariffActions" :key="a.value" :value="a.value">{{ a.label }}</option>
                    </select>
                    <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">
                        Přiřadit službu
                    </button>
                    <div v-if="tariffForm.errors.tariff_id || tariffForm.errors.action" class="text-red-600 text-sm mt-2">
                        <div v-if="tariffForm.errors.tariff_id">{{ tariffForm.errors.tariff_id }}</div>
                        <div v-if="tariffForm.errors.action">{{ tariffForm.errors.action }}</div>
                    </div>
                </form>
            </div>
            ...
            <div v-if="tab=='csv'">
                <div class="mb-4">
                    <label class="font-semibold block mb-1">Nahrát CSV soubor</label>
                    <input type="file" accept=".csv" @change="handleCSVFile" class="border p-2 rounded"/>
                </div>
                <div v-if="columns.length">
                    <label class="font-semibold block mb-1">Vyberte sloupec se službami</label>
                    <select v-model="csvTariffColumn" class="border p-2 w-full rounded mb-2">
                        <option value="" disabled>Vyberte sloupec</option>
                        <option v-for="col in columns" :key="col" :value="col">{{ col }}</option>
                    </select>
                    <button v-if="csvTariffColumn" @click="getUniqueTariffsFromCSV"
                            class="bg-blue-500 text-white px-4 py-1 rounded mb-4">
                        Načíst unikátní hodnoty
                    </button>
                </div>
                <div v-if="uniqueTariffs.length">
                    <h4 class="font-bold mb-2">Unikátní služby nalezené v CSV:</h4>
                    <ul class="max-h-40 overflow-auto border rounded mb-4 p-2 bg-gray-50">
                        <li v-for="t in uniqueTariffs" :key="t">{{ t }}</li>
                    </ul>
                    <button @click="saveUniqueTariffs"
                            class="bg-green-500 text-white px-6 py-2 rounded hover:bg-green-600 mb-4"
                            :disabled="uploading">
                        Uložit do databáze
                    </button>
                </div>
